...
# ----------------------------------------------- the admin view is wired up


#: Every renderer the admin view depends on, and the function that must call it.
#: This exists because a whole batch of these rewrites was once applied to a temp
#: file that a later step overwrote — the helpers were present and simply never
#: called, the console rendered its old behaviour, and the suite stayed green.
#: Defining a function is not the same as reaching it.
ADMIN_WIRING = [
    ("renderAdmin", "renderStamp("),
    ("renderAdmin", "renderReceipt("),
    ("renderAdmin", "renderPipeline("),
    ("renderAdmin", "renderRetrieval("),
    ("renderRetrieval", "inspectFind("),
    ("inspectFind", "renderLineage("),
    ("inspectFind", "renderEvidence("),
    ("renderRecordLog", "renderRunTiles("),
    ("renderRecordLog", "tokenNote("),
    ("renderPipeline", "pipeMeta"),
    ("bindDynamicActions", "data-inspect-find"),
]


@pytest.mark.parametrize("caller, callee", ADMIN_WIRING)
def test_the_admin_view_reaches_every_panel_it_defines(code, caller, callee):
    assert callee in body_of(code, caller), f"{caller}() never reaches {callee}"


def test_the_console_makes_no_liveness_claim(code):
    """A pulsing dot over a static build asserted something the page could not
    support. The stamp and the receipt are what replaced it."""
    assert "con-live" not in code
    assert "con-pulse" not in code
    assert "adminStamp" in body_of(code, "renderStamp")


def test_a_run_with_no_model_call_is_never_reported_as_zero(code):
    """The Python guards the column; this guards the sentence printed from it."""
    source = body_of(code, "tokenNote")
    assert "no model call" in source
    assert "tokens not recorded" in source
